...
read.csvODK <- function(base_name = "censoPeic",  form_rep = c("subquad", "tree", "sec_fuste", "rep_miss"), save_file = TRUE, dir_exp = getwd(), file_name = NULL)
{
    ...
    treenames <- gsub("conf_diff.", "", treenames)
    ...
    iMap <- which(newMap)
    treequad$new_dy2018 <- treequad$new_dx2018 <- NA 
for(i in iMap)
    {
...
### Incluir uma saida de objeto em lista com o treemiss e tree
    if(save_file)
    {
        ...
        file.rename(from = names.csv, to=  file.path(dname, "baseData", paste("censoPeicNoMiss",c("",form_rep), qname,".csv", sep="")))
...
## abaixo os comandos da funcao 'subquad'
            pos <- seq(5,15, by= 5)
            dx.ind <- apply(as.matrix(sapply(pos, function(x){ifelse(denc$new_dx2018 >= x, 5,0)})), 2, sum)
            dy.ind <- apply(as.matrix(sapply(pos, function(x){ifelse(denc$new_dy2018 >= x, 5,0)})), 2, sum)
...
#table(tree$tree_type)
    daps <- as.numeric(tree$dap2018[tree$tree_type == "sem_info" & is.na(tree$tree_tag)])
...
{
## this is full names from data, related with the last odk form and odkexp function from 2018 december 12 
    treenames <- c("key_quad", "today", "quadrat", "quad5x5", "n_tree", "tree_type", "tag_ok", "tag_dead", "arvmap_key", "confTagMap", "tree_tag_map", "tree_tag", "old_dap", "old_alt", "old_dx", "old_dy", "old_fam", "old_sp", "old_nfuste", "arvmap_conf", "newmap_codpos", "new_tag", "new_tag_picture", "num_tag", "alt_max0", "alt_max1", "tipo_med", "dap2018_mm", "dap2018_cm", "pap2018_cm", "one_fuste", "info_id", "new_fam", "new_gen", "new_sp", "coleta", "id_same", "id_picture", "map2018", "dap2018", "alt2018", "fam2018", "sp2018", "conf_ok", "obs_tree", "tagdead_label", "difdapOK", "difaltOK", "key_tree", "new_dy2018", "new_dx2018", "nfuste2018", "dap2018_sec_mm")
    resnames <- c("today", "tree_type", "tag_ok", "tree_tag", "new_tag", "num_tag", "tag_dead", "quadrat", "quad5x5", "old_dx", "old_dy", "new_dx2018", "new_dy2018", "old_alt", "alt2018", "old_nfuste", "nfuste2018", "old_dap", "dap2018", "dap2018_sec_mm", "old_fam", "old_sp", "fam2018", "sp2018", "coleta", "difdapOK", "difaltOK", "obs_tree")
...
#########################################
    ...
    for(i in dupltag)
    {
        ...
        dupldata <- tree1[postag, c("dap2018","alt2018", "fam2018", "sp2018")]
        dupligual <- c(dupligual,postag[duplicated(dupldata)])
    }
